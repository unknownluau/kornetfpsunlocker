...
std::vector<HANDLE> GetKornetProcesses(bool include_client = true, bool include_studio = true)
{
	std::vector<HANDLE> result;
	if (include_client)
	{
		for (HANDLE handle : ProcUtil::GetProcessesByImageName("KornetPlayerBeta.exe")) result.emplace_back(handle);
	}
	if (include_studio) for (HANDLE handle : ProcUtil::GetProcessesByImageName("KornetStudioBeta.exe")) result.emplace_back(handle);
	return result;
}

HANDLE GetKornetProcess()
{
	auto processes = GetKornetProcesses();
...
const void *FindTaskScheduler(HANDLE process, const char **error = nullptr)
{
	try
	{
		...
		while (true)
		{
			...
			{
				if (error) *error = "Failed to get process base! Please restart Kornet FPS Unlocker.";
				return nullptr;
			}
		}
...
struct KornetProcess
{
	...
	void Tick()
	{
		...
		if (!ts_ptr)
		{
			...
			if (!ts_ptr)
			{
				...
				if (retries_left-- <= 0)
					NotifyError("kornetfpsunlocker Error", error ? error : "Unable to find TaskScheduler! This is probably due to a Kornet update-- watch the github for any patches or a fix.");
				return;
			}
		}
		...
		if (ts_ptr && !fd_ptr)
		{
			try
			{
				if (auto scheduler = (const uint8_t *)(ProcUtil::ReadPointer(handle, ts_ptr)))
				{
					...
					{
						if (retries_left-- <= 0)
							NotifyError("kornetfpsunlocker Error", "Variable scan failed! This is probably due to a Kornet update-- watch the github for any patches or a fix.");
						return;
					}
			...
			{
				printf("[%p] KornetProcess::Tick failed: %s (%d)\n", handle, e.what(), e.GetLastError());
				if (retries_left-- <= 0)
					NotifyError("kornetfpsunlocker Error", "An exception occurred while performing the variable scan.");
			}
		}
	}
	...
	void SetFPSCap(double cap)
	{
		if (fd_ptr)
		{
			...
			{
				printf("[%p] KornetProcess::SetFPSCap failed: %s (%d)\n", handle, e.what(), e.GetLastError());
			}
		}
	}
};

std::unordered_map<DWORD, KornetProcess> AttachedProcesses;
...
DWORD WINAPI WatchThread(LPVOID)
{
	...
	while (1)
	{
		auto processes = GetKornetProcesses(Settings::UnlockClient, Settings::UnlockStudio);
		...
		for (auto& process : processes)
		{
			...
			if (AttachedProcesses.find(id) == AttachedProcesses.end())
			{
				...
				KornetProcess kornet_process;

				kornet_process.Attach(process, 3);

				AttachedProcesses[id] = kornet_process;
...
int CALLBACK WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	if (UI::IsConsoleOnly)
	{
		...
		if (!KornetProcess().Attach(process, 0))
		{
	...
	else
	{
		if (CheckRunning())
		{
			MessageBoxA(NULL, "Kornet FPS Unlocker is already running", "Error", MB_OK);
		}
		else
		{
